...
Camera::Camera(std::string name, float fov_x): name_{name}, fov_x_{fov_x}, eye_{0.0,0.0,0.0}, dir_{0.0,0.0,-1}, up_{0.0,1,0.0}{}
...
Camera::Camera(): name_{""}, fov_x_{0.0f}, eye_{0.0,0.0,0.0}, dir_{0.0,0.0,-1}, up_{0.0,1,0.0}{}
Camera::~Camera(){}
...
void Camera::generate_rays(unsigned width, unsigned height, std::vector<Ray> & rays){
	...
	for (int y = ((int)height/-2); y < (int)height/2; ++y)
	{
		for (int x = ((int)width/-2); x < (int)width/2; ++x)
		{
			...
			//Verrechnung mit der Matrix zur Kameraposition wenn nicht default
				...
				up_.x != 0 || up_.y != 1 || up_.z != 0 )
			{
				...
				dir_ = glm::normalize(dir_);
				//Matrix wird wird Spalte x Zeile implementiert
